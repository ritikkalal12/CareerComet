...
def job_list_view(request):
    try:
        ...
        print(f"Error in job_list_view: {e}")
        # Handle the error appropriately, such as returning an error page or logging it
...
def feedback_redirect(request):
    ...
    if user_role == 'employee':
        return redirect('jobapp:employee_feedback_form')
    elif user_role == 'employer':
        return redirect('jobapp:employer_feedback_form')
    else:
        # Handle other cases, e.g., if the user is neither employee nor employer
        return render(request, 'jobapp/feedback_redirect_error.html')



def employee_feedback_form(request):
    if request.method == 'POST':
        form = EmployeeFeedbackForm(request.POST)
        if form.is_valid():
            form.save()
            return redirect('jobapp:thank_you')
    else:
        form = EmployeeFeedbackForm()

    return render(request, 'jobapp/employee_feedback_form.html', {'form': form})

def employer_feedback_form(request):
    if request.method == 'POST':
        form = EmployerFeedbackForm(request.POST)
        if form.is_valid():
            form.save()
            return redirect('jobapp:thank_you')
    else:
        form = EmployerFeedbackForm()

    return render(request, 'jobapp/employer_feedback_form.html', {'form': form})


def thank_you(request):
    return render(request, 'jobapp/thank_you.html')


def subscription(request):
    # Your view logic here
    return render(request, 'jobapp/subscription.html')
